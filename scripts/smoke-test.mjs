...
async function testMobileReadiness() {
  ...
  const checks = [
    ...
    ['Story create portal', readFileSync(join(process.cwd(), 'frontend', 'src', 'components', 'StoryCreateModal.tsx'), 'utf8').includes('createPortal') && readFileSync(join(process.cwd(), 'frontend', 'src', 'index.css'), 'utf8').includes('.app-modal-overlay')],
    ['Modal portal layer', existsSync(join(process.cwd(), 'frontend', 'src', 'components', 'ModalPortal.tsx')) && readFileSync(join(process.cwd(), 'frontend', 'src', 'components', 'ActionInfoModal.tsx'), 'utf8').includes('ModalPortal')],
    ...
    ['iPhone 15–17 E2E matrix', existsSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'))],
    ['12 iPhone modelos certificados', readFileSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'), 'utf8').includes('iPhone 17 Pro Max') && readFileSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'), 'utf8').includes('iPhone 16')],
    ['iOS 26 y 27 beta UA', readFileSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'), 'utf8').includes('iOS 26') && readFileSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'), 'utf8').includes('iOS 27 beta')],
    ['Android variado E2E', readFileSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'), 'utf8').includes('Galaxy S9+') && readFileSync(join(process.cwd(), 'scripts', 'e2e-mobile-devices.mjs'), 'utf8').includes('Pixel 7')],
    ...
    ['boot-splash fuera de #root', /id="boot-splash"[\s\S]*id="root"/.test(readFileSync(join(process.cwd(), 'frontend', 'index.html'), 'utf8'))],
    ['boot recovery inline', readFileSync(join(process.cwd(), 'frontend', 'index.html'), 'utf8').includes('reto_boot_reload')],
    ['PageTransition initial=false', readFileSync(join(process.cwd(), 'frontend', 'src', 'components', 'PageTransition.tsx'), 'utf8').includes('initial={false}')],
    ['chunkRecovery module', existsSync(join(process.cwd(), 'frontend', 'src', 'lib', 'chunkRecovery.ts'))],
    ['setup server flag', readFileSync(join(process.cwd(), 'backend', 'prisma', 'schema.prisma'), 'utf8').includes('setupCompletedAt')],
    ['PWA no auto modal tras setup', readFileSync(join(process.cwd(), 'frontend', 'src', 'components', 'PwaInstallGate.tsx'), 'utf8').includes('install.promptOpen')],
    ['setup sync login', readFileSync(join(process.cwd(), 'frontend', 'src', 'pages', 'Login.tsx'), 'utf8').includes('syncSetupFromUser')],
